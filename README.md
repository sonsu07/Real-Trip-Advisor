...
### 회원가입 gender 선택 중복 불가 처리
분기처리로 중복되지 않도록 세팅

---


### fly.io를 통해 json 데이터 별도로 배포
로컬로 설정해 둔 데이터를 서버에 올림.
도중에 데이터가 분실된 링크들 재구성.

---

### cors 문제 해결
서버단에서 cors 허용 세팅
https://real-trip-advisor-homepage.fly.dev 주소를 허용하도록 설정해 둠.

### 어디가 문제였나? 
로컬에서 실행시키는 경우 IDE에서 자체적으로 켜주는 호스트 주소를 통해 구동시키다보니 CORS 문제가 생긴것도 최근에 알아채버렸다. 
이를 클라이언트단에서 해결하기위해 여러가지 방법을 시도해 보았다. 

우선 CORS 문제가 왜 발생하는지 잠깐 정리해보자면
흔히 사용하는 웹 주소는 기본적으로 스킴, 호스트, 포트 + 경로, 쿼리스트링, Fragment로 구성되어 있는데,
이 중에서 스킴과 호스트, 포트 번호가 같아야지 동일한 출처로 인정하여 클라이언트에서 데이터를 응답받는다. 

이러한 특징은 교차출처를 막 허용해줄 경우, 다른 도메인에서 우리가 사용하는 도메인으로 임의의 스크립트를 삽입하여 데이터를 빼가거나 하는 
해킹 수법을 막기위해 고안된 방식이라고 볼 수 있다. 따라서 보안적인 관점에서는 필수적으로 지켜줘야하는 방식이다. 

이를 해결하는 방법은 기본적으로 서버단에서 Access-Control-Allow-Origin이라는 필드를 헤더에 추가하고, 해당 value값으로 허용할 웹 사이트의 도메인 혹은 호스트 주소를
삽입해주면 된다. 다만, 본 프로젝트의 경우에는 초기에 서버단을 전혀 고려하지 않았기에, 클라이언트단에서 해당 문제를 해결하기 위해 고심하였다. 
결과적으로 로컬 서버도 존재하지 않은 상황에, 서버를 연다고 치면 http-server 라이브러리를 통해 열어서 접속을 시도하여야 했고, 이같은 경우 cors 허용 세팅이 잘 이루어지지 않았기에 
어쩔수 없이 express로 라우팅 작업을 거쳐 배포까지 실행하였다. 

CORS 문제를 어렴풋이는 알고 있었지만, 사실 클라이언트단에서만으로 해결될거라고 막연히 생각했는데... 그냥 빽단 작업자한테 요청하는게 빠르게 처리될것 같다. 

### 살펴볼 개념
CORS 관련 MDN 문서를 시간나면 정리해볼 것


## 12.28
### express 추가 및 fly.io를 이용한 배포 완료
express의 router 기능을 이용하여 라우팅 기능을 활성화시키고, 이를 통해 fly.io에서 배포 완료